            ...
            <tr class="hover:bg-gray-50 transition">
                ...
                <td class="p-4 border-b text-center">
                    <button onclick="openStudentModal('{{ $student->id }}')" 
                        class="bg-indigo-100 text-indigo-700 px-4 py-1.5 rounded-full text-xs font-bold hover:bg-indigo-600 hover:text-white transition shadow-sm">
                        Lihat Detail
                    </button>
                </td>
            </tr>
...
<script>
    ...
    function renderChart(titles, progress) {
        ...
        const options = {
            series: [{ name: 'Progres', data: progress }],
            chart: { 
                type: 'bar', 
                height: dynamicHeight, // Menggunakan tinggi dinamis
                toolbar: { show: false } 
            },
            plotOptions: { 
                bar: { 
                    ...
                    borderRadius: 5, 
                    distributed: true,
                    barHeight: '70%' // Membuat batang sedikit lebih tebal
                } 
            },
            xaxis: { categories: titles, max: 100 },
            // ... warna dan opsi lainnya tetap sama ...
        };
